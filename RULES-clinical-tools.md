...
> Dernière mise à jour : mars 2026 (v1.1 — ajout règles données/outcomes/sources/PDF)
...
> **Règle implicite :** chaque algorithme clinique donne lieu aux **deux artefacts** en parallèle. Ne créer qu'un seul des deux n'est pas la norme.
...
**Mode sombre** → outils de stratification (Type A)
```css
--bg:       #0d1117;
--surface:  #161b22;
--surface2: #1c2330;
...
**Mode clair / parchemin** → logigrammes D3 (Type B)
```css
--bg:      #f5f0e8;
--surface: #ede8dc;
--border:  #c8bfaa;
...
5. **Bouton reset** (logigrammes) OU rien (outils de stratification)

**Header sticky** dans les outils de stratification :
```css
...
**Header fixe** dans les logigrammes :
```css
...
Présente en bas de page (outils de stratification) ou dans un panneau dédié (logigrammes via modal/onglet). Structure :
...
<div class="ref-item">
  ...
  <div class="ref-note">Note contextuelle optionnelle (données clés de l'étude)</div>
</div>
```
...
### Études princeps à toujours citer (médulloblastome)
Selon le groupe de risque et le protocole, citer en priorité :
- L'essai randomisé fondateur du protocole (ex. ACNS0331, PNET5-MB, HIT-SKK 2000)
- La méta-analyse ou série la plus récente donnant les taux d'OS/EFS
- L'étude de toxicité tardive si des données protons/photons sont mentionnées

### Affichage des superscripts dans le contenu
```html
<!-- Dans un tooltip ou une card : -->
OS 5 ans : 80–85 % <sup class="ref-sup">[1]</sup>
<!-- Couleur text3, font-size 8px, cursor pointer → scroll vers la référence -->
```
...
Bouton présent dans la **card de résultat final uniquement** (pas dans les étapes intermédiaires) :
...
function printResult() {
  // Masquer tout sauf la card résultat + header + références
...
@media print {
  ...
  .btn-pdf, .btn-reset, .progress-wrap { display: none !important; }
  ...
  /* Masquer les liens DOI trop longs — optionnel */
  .ref-doi a::after { content: none; }
}
```

### Contenu imprimé (ce qui doit apparaître)
1. Header réduit (logo + titre + date d'impression)
2. Card de résultat complet (protocole, doses RT, chimio, outcomes, toxicités)
3. Section références correspondantes (uniquement celles citées dans le résultat)
4. Mention de non-substitution à la décision médicale (footer)
...
- Jamais de couleur codée en dur dans les règles CSS (sauf rgba avec valeur hex en fallback dans les backgrounds)
...
```
Couleur du badge = couleur distinctive du protocole (purple pour SFCE, blue pour SIOP, orange pour ARIA).
...
.result-card {
  ...
  border-left: 3px solid var(--RISK-COLOR);   /* ou border-top */
...
- [ ] Le `@media print` masque les éléments de navigation
